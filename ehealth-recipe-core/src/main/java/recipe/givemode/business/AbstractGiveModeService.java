...
public abstract class AbstractGiveModeService implements IGiveModeBase{
    ...
    public void afterSetting(GiveModeShowButtonVO giveModeShowButtonVO, Recipe recipe) {
        ...
        RecipeDistributionFlagEnum recipeDistributionFlagEnum = RecipeDistributionFlagEnum.getRecipeDistributionFlagEnum(recipe.getDistributionFlag());
        if(Objects.nonNull(recipeDistributionFlagEnum)) {
            switch (recipeDistributionFlagEnum) {
                case HOS_HAVE:

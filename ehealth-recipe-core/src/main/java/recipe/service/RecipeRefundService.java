...
public class RecipeRefundService extends RecipeBaseService{
    ...
    public void refundResultCallBack(RefundRequestBean refundRequestBean) {
        ...
        if (refundRequestBean != null && StringUtils.isNotEmpty(refundRequestBean.getRecipeCode())) {
            ...
            if (recipeOrder.getDispensingFlag().equals(1)) {
                LOGGER.info("RecipeRefundService.refundResultCallBack OrderCode = {}", recipe.getOrderCode());
                throw new DAOException(ErrorCode.SERVICE_ERROR, "发药标示已发药，请操作退药");
            }
    ...
    public List<RecipeRefundBean> findRecipeReFundRate(Integer recipeId) {
        ...
        if (CollectionUtils.isEmpty(applyRefunds)) {
            return null;
        }
        RecipeRefund applyRefund = applyRefunds.get(0);

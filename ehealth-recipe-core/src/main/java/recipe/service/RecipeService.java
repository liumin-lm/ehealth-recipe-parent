    ...
    public void retryDoctorSignCheck(Integer recipeId) {
        ...
        try {
            ...
            if (RecipeResultBean.FAIL.equals(recipeSignResult.getCode())) {
                //说明处方签名失败
                recipeDAO.updateRecipeInfoByRecipeId(recipeId, RecipeStatusConstant.SIGN_ERROR_CODE_DOC, null);
                recipeLogDAO.saveRecipeLog(recipeId, recipe.getStatus(), recipe.getStatus(), "说明处方签名失败:" + recipeSignResult.getMsg());
                //CA同步回调的接口 发送环信消息
        ...
        //TODO 根据审方模式改变状态
        //设置处方签名成功后的处方的状态
        auditModeContext.getAuditModes(recipe.getReviewType()).afterHisCallBackChange(status, recipe, "当前签名处方签名成功");
...
//            result.remove("medicines");
     ...
     * @return
     */
    public Map<String, Object> drugInfoSynMovementExtD(Integer organId, List<String> drugForms, Map<String, OrganDrugList> drugMap, String operator, Boolean sync, Boolean add, Boolean commit, Boolean delete) throws ParseException {
        SimpleDateFormat myFmt2 = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        Map<String, Object> map = Maps.newHashMap();
        //List<OrganDrugInfoTO> finalData = data;
        IRecipeHisService recipeHisService = AppDomainContext.getBean("his.iRecipeHisService", IRecipeHisService.class);

...
import recipe.prescription.service.PrescriptionService;
...
public class RecipeService {
    ...
    public void saveRecipeDocIndex(Recipe recipe) {
        ...
        docIndex.setDoctorName(iDoctorService.getNameById(recipe.getDoctor()));
    ...
    public Map<String, Object> doSignRecipe(Recipe recipe, List<Recipedetail> details) {
        ...
        PatientBean requestPatient = iPatientService.findOwnPatientByMpiId(patient.getLoginId());
    ...
    public Map<String, Object> doSignRecipeExt(Recipe recipe, List<Recipedetail> details) {
        ...
        if (result) {
            ...
            if (!recipe.canMedicalPay()) {
                RecipeServiceSub.sendRecipeTagToPatient(recipe, details, rMap);
            }
        }
    ...
    public RecipeResultBean afterCheckPassYs(Recipe recipe) {
        ...
        if (recipe.canMedicalPay()) {
            ...
            RecipeServiceSub.sendRecipeTagToPatient(recipe, detailDAO.findByRecipeId(recipeId), null);
            //向患者推送处方消息
    ...
    public void drugInfoSynTask(Integer organId) {
        ...
        if (CollectionUtils.isNotEmpty(organIds)) {
            for (int _organId : organIds) {
                ...
                do {
                    unuseDrugs.clear();
                    List<DrugInfoTO> drugInfoList = hisService.getDrugInfoFromHis(_organId, false, startIndex);
...
//                            organDrugListDAO.updateStatusByOrganDrugCode(unuseDrugs, 0);
                        ...
                        logger.error("drugInfoSynTask organId=[{}] total=[{}] 药品信息更新结束.", startIndex, _organId);
                        finishFlag = false;
                    }
                } while (finishFlag);
            }
    ...
    public boolean supportTakeMedicine(Integer recipeId, Integer clinicOrgan) {
        ...
        boolean flag = iOrganService.getTakeMedicineFlagById(clinicOrgan);
        //是否支持医院取药 true：支持
        if (flag) {
                ...
                if (RecipeBussConstant.GIVEMODE_SEND_TO_HOME.equals(giveMode)) {
                    if (RecipeBussConstant.PAYMODE_ONLINE.equals(payMode)) {
                        ...
                        if (PayConstant.PAY_FLAG_PAY_SUCCESS == payFlag) {
                            ...
                            if (RecipeUtil.isTcmType(_dbRecipe.getRecipeType())) {

...
public class HisAdministrationRemoteService extends AccessDrugEnterpriseService {
    ...
    @Override
    //his管理的药企：药品，库存，价格全都由；医院返回的信息提供，判断库存现阶段由预校验的结果判断
    ...
    @Override
    //医院管理药企，查询药企列表，按his返回信息展示，现阶段是预校验的时候返回的信息
    ...
    @Override
    //his管理的药企：设置app端的额外药企信息；医院返回的信息提供，现阶段由预校验结果展示
    ...
    @Override
    //his管理的药企：设置订单时候产生的金额；医院返回的信息提供，现阶段由预校验结果存储
    ...
    @Override
    //his管理的药企：确认订单时候展示药企信息；医院返回的信息提供，现阶段由预校验结果存储
    ...
    @Override
    //his管理的药企：将处方对应支持的药企信息关联处方；现阶段在【预校验】的时候，将his产生的药企信息放入扩展表中
    ...
    @Override
    //his管理的药企：订单生成的时候设置药企信息；将预校验产生的药企信息关联到订单上
    ...
    @Override
    //his管理的药企：药企筛选列表；his维护的药企不在平台维护，需要去除不用的虚拟药企
    ...
    @Override
    //his管理的药企：配送信息同步到his，现阶段是his去做的同步，之后的版本可能会改动
    ...
    //**这个接口之所以放在这里是因为，现阶段是将同步配送信息放置在【预结算】之后，可以根据之后的实现调整

...
public class HzInternetRemoteService extends AccessDrugEnterpriseService {
    ...
    public DrugEnterpriseResult recipeMedicalPreSettle(Integer recipeId, Integer depId) {
        ...
        if (recipeId == null || depId == null) {
            ...
            LOGGER.info("recipeMedicalPreSettle-杭州互联网医保预结算开始,处方号={},药企ID：{},medicalPayConfig={}", recipeId, depId, medicalPayConfig);
            //杭州市互联网或者打开医保支付开关的互联网(杭州市三等走自费预结算)
            if (drugEnterprise != null && "hzInternet".equals(drugEnterprise.getAccount()) || (medicalPayConfig && RecipeBussConstant.RECIPEMODE_ZJJGPT.equals(recipe.getRecipeMode()))) {
                ...
                //有市名卡才走预结算--因为对于杭州互联网是必填的
                if (StringUtils.isEmpty(bxh) && drugEnterprise != null && "hzInternet".equals(drugEnterprise.getAccount())) {

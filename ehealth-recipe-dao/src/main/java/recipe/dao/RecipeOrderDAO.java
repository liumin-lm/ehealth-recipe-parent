...
public abstract class RecipeOrderDAO extends HibernateSupportDelegateDAO<RecipeOrder> implements ExtendDao<RecipeOrder> {
    ...
    @DAOMethod(sql = "from RecipeOrder where orderId in (:orderList)")
    public abstract List<RecipeOrder> findByOrderId(@DAOParam("orderList") Collection<Integer> orderIdList);
    ...
    protected StringBuilder generateRecipeOrderDetailHQL(RecipeOrderRefundReqDTO recipeOrderRefundReqDTO){
        ...
        hql.append("case when a.orderType in (1,2,3,4) then '医保' else '自费' end as orderTypeText,a.fundAmount,a.cashAmount,a.refund_amount as refundAmount,");

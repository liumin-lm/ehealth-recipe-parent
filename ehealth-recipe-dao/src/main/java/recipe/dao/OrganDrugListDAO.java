...
public abstract class OrganDrugListDAO extends HibernateSupportDelegateDAO<OrganDrugList> implements DBDictionaryItemLoader<OrganDrugList>, ExtendDao<OrganDrugList> {
    ...
    public abstract List<OrganDrugList> findByOrganIdWithOutStatus(@DAOParam("organId") int organId);
     ...
     * @return
     */
    @DAOMethod(limit = 0, sql = "from OrganDrugList where organId=:organId and drugId in (:drugIds) and status=1")
    public abstract List<OrganDrugList> findByOrganIdAndDrugIds(@DAOParam("organId") int organId, @DAOParam("drugIds") Collection<Integer> drugIds);
    ...
    public abstract List<OrganDrugList> findByOrganIdAndDrugIdsAndStatus(@DAOParam("organId") int organId, @DAOParam("drugIds") Collection<Integer> drugIds);
    ...
    @DAOMethod(sql = "from OrganDrugList where organDrugCode=:organDrugCode and organId=:organId ")
    public abstract List<OrganDrugList> findByOrganDrugCodeAndOrganIdAndAllStatus(@DAOParam("organDrugCode") String organDrugCode, @DAOParam("organId") int organId);
    ...
    @DAOMethod(sql = "from OrganDrugList where organDrugCode in:organDrugCodes and organId in:organIds", limit = 0)
    public abstract List<OrganDrugList> findByOrganDrugCodesAndOrganIds(@DAOParam("organDrugCodes") List<String> organDrugCodes, @DAOParam("organIds") List<Integer> organIds);
        ...
        HibernateStatelessResultAction<QueryResult<DrugListAndOrganDrugList>> action = new AbstractHibernateStatelessResultAction<QueryResult<DrugListAndOrganDrugList>>() {
            ...
            public void execute(StatelessSession ss) throws DAOException {
                if (status == 2) {
                    ...
                    Query countQuery = ss.createQuery("select count(*) " + hql.toString());
                    if (!StringUtils.isEmpty(drugType)) {
                        countQuery.setParameter("drugType", Integer.valueOf(drugType));
                    }
                    if (!StringUtils.isEmpty(producer)) {
                    ...
                    Query query = ss.createQuery("select d " + hql.toString());
                    if (!StringUtils.isEmpty(drugType)) {
                        query.setParameter("drugType", Integer.valueOf(drugType));
                    }
                    if (!StringUtils.isEmpty(producer)) {
...
//                            }
    ...
    @DAOMethod(sql = "from OrganDrugList where organId=:organId ", limit = 0)
    public abstract List<OrganDrugList> findOrganDrugByOrganIdAndAllStatus(@DAOParam("organId") int organId);
     ...
     * @return
     */
    @DAOMethod(sql = "from OrganDrugList where organId=:organId   and regulationDrugCode is null ", limit = 0)
    ...
    public List<OrganDrugList> listOrganDrugByTime(ListOrganDrugReq param) {
        HibernateStatelessResultAction<List<OrganDrugList>> action = new AbstractHibernateStatelessResultAction<List<OrganDrugList>>() {
            @Override
            public void execute(StatelessSession ss) throws Exception {
                StringBuilder hql = new StringBuilder();
                hql.append("select r from OrganDrugList r where organid =:organId ");
                if (org.apache.commons.lang3.StringUtils.isNotEmpty(param.getStartDate())) {
                    hql.append(" and r.lastModify >= :startTime");
                }
                if (org.apache.commons.lang3.StringUtils.isNotEmpty(param.getEndDate())) {

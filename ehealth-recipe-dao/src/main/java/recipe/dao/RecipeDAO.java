...
//                    hql.append(" and c.checkStatus =").append(0).append(" and r.checker is not null ");
    ...
    public List<PatientRecipeBean> findTabStatusRecipesForPatientNew(final List<String> mpiIdList, final int start, final int limit, final List<Integer> recipeStatusList, final List<Integer> orderStatusList, final String tabStatus) {
        HibernateStatelessResultAction<List<PatientRecipeBean>> action = new AbstractHibernateStatelessResultAction<List<PatientRecipeBean>>() {
            @Override
            public void execute(StatelessSession ss) throws Exception {
                StringBuilder hql = new StringBuilder();
                ...
                if ("onready".equals(tabStatus)) {
                    hql.append("select 1 as type,t.RecipeID as recordCode,t.RecipeID as recordId,t.MPIID as mpiId,t.OrganDiseaseName as diseaseName,(case when (t.reviewType = 1 and t.checkStatus = 1 and t.status = 15) then 8 else t.Status end),t.TotalMoney as fee," + "t.SignDate as recordDate,null,t.MedicalPayFlag as medicalPayFlag,t.RecipeType as recipeType,t.ClinicOrgan as organId,t.recipeMode as recipeMode,s.giveMode, t.recipeSource as recipeSource,t.payFlag as payFlag ,t.recipeId from cdr_recipe t ");

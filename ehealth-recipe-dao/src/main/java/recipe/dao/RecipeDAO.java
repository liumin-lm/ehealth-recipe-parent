        ...
        HibernateStatelessResultAction<List<Recipe>> action = new AbstractHibernateStatelessResultAction<List<Recipe>>() {
            ...
            public void execute(StatelessSession ss) throws Exception {
                ...
                //3是全部---0409小版本要包含待审核或者审核后已撤销的处方
                    ...
                    hql.append("select r.* from cdr_recipe r where r.clinicOrgan in (:organ) and r.checkMode<2   and (r.audit_state in (1,2,3,4,5,6,7) || (r.audit_state=0 && r.support_mode=1))  and  (r.recipeType in(:recipeTypes) or r.grabOrderStatus=1) and r.reviewType != 0 ");

        ...
        HibernateStatelessResultAction<List<Recipe>> action = new AbstractHibernateStatelessResultAction<List<Recipe>>() {
            ...
            public void execute(StatelessSession ss) throws Exception {
                ...
                //3是全部---0409小版本要包含待审核或者审核后已撤销的处方
                    ...
                    hql.append("select * from cdr_recipe r where r.clinicOrgan in (:organ) and (r.status = 8 or r.checkDateYs is not null or (r.status = 9 and (select l.beforeStatus from cdr_recipe_log l where l.recipeId = r.recipeId and l.afterStatus =9 ORDER BY l.Id desc limit 1) in (8,15,7,2))) ");
                } else {
                    throw new DAOException(ErrorCode.SERVICE_ERROR, "flag is invalid");
                }

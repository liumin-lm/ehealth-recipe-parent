...
public abstract class SymptomDAO extends HibernateSupportDelegateDAO<Symptom> {
     ...
     * @return
     */
    @DAOMethod(sql = "from Symptom where organId=:organId and symptomCode in:symptomCodes  ",limit =0)
    public abstract List<Symptom> findByOrganIdANDCodes(@DAOParam("organId") Integer organId,@DAOParam("symptomCodes") List<String> symptomCodes);
            ...
            public void execute(StatelessSession ss) throws Exception {
                ...
                if (!ObjectUtils.isEmpty(isRegulationSymptom)){
                    if (isRegulationSymptom){
                        sql.append(" and regulationSymptomCode is not null and regulationSymptomCode <> '' ");
                    }else {
                        sql.append(" and ( regulationSymptomCode is  null  or regulationSymptomCode = '' ) ");
                    }
                }
